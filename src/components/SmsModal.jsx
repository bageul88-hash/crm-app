  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ width: 40, height: 4, background: 'var(--border2)', borderRadius: 2, margin: '12px auto 0' }} />
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text3)', letterSpacing: '0.05em', textTransform: 'uppercase', display: 'block', marginBottom: 8 }}>문자 종류 선택</label>
          ...
          <div style={{ marginBottom: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text3)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>미리보기</label>
              <button onClick={() => setEditing(!editing)} style={{ background: 'none', border: 'none', color: 'var(--accent)', fontSize: 13, cursor: 'pointer', fontFamily: 'var(--font)' }}>
                {editing ? '✓ 완료' : '✏️ 수정'}
              </button>
            </div>
            ...
            {editing ? (
              <textarea value={body} onChange={e => setBody(e.target.value)} style={{
                ...
                lineHeight: 1.7, padding: 14, resize: 'none', outline: 'none', boxSizing: 'border-box',
              }} />
            ) : (
              <div style={{
                background: 'var(--bg3)', border: '1px solid var(--border)',
                borderRadius: 'var(--radius-sm)', padding: 14, fontSize: 13,
                lineHeight: 1.8, color: 'var(--text)', whiteSpace: 'pre-wrap',
                maxHeight: 320, overflowY: 'auto',
              }}>{body}</div>
            )}
            <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 6, textAlign: 'right' }}>{body.length}자</div>
          </div>

          {(consult.diagDate || consult.diagTime) && (
            <div style={{
              background: 'rgba(79,126,248,0.08)', border: '1px solid rgba(79,126,248,0.2)',
              borderRadius: 'var(--radius-sm)', padding: '10px 14px', fontSize: 12, color: 'var(--text2)', marginBottom: 8,
            }}>
              ✅ 자동 입력됨:
        ...
        <div style={{ padding: '12px 20px 28px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10 }}>
          ...
          <button className="btn btn-primary" style={{ flex: 2, fontSize: 15 }} onClick={handleSend} disabled={sent}>
            {sent ? '✅ 문자앱 열림' : '📱 문자 보내기'}
          </button>
        </div>
      </div>
